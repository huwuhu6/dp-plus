...
## 2026-08-27：替代推荐运行级指标

`39 条轨迹、85 个对话轮次` 是当前启用评测数据集的规模，不是成功率。其中主集为 19 条/40 轮，留出集为 15 条/32 轮，鲁棒集为 5 条/13 轮。成功率必须由一次实际运行产生的分子和分母计算，不能从数据集规模推导。

V48 为每次对话评测运行增加 `unseenRecommendationExpectedCount` 与 `unseenRecommendationMatchedCount`，并在运行对比接口输出 `unseenRecommendationMatchRate` 的变化值。该指标的分母仅为声明了“候选不得重复”断言的轨迹，分子为实际通过这些断言的轨迹；没有该断言的普通对话不会稀释结果。V47 的六条新增替代推荐轨迹和鲁棒集城市切换轨迹将在下一次真实主集、留出集、鲁棒集回放中形成可追踪的真实分母与通过率。

本次实现验证：`mvn -q test` 完整运行，118 个测试通过、0 失败、0 错误、1 个既有跳过项；`AiConversationEvaluationServiceTest` 11/11 通过，覆盖运行级分子/分母聚合和基线对比增量。真实模型回放尚未重新执行：本机没有运行中的应用服务，也没有可用登录态；不绕过认证、不复用用户会话数据。待在已登录的运行环境提交评测后，开发记录必须补充运行 ID、数据集版本、模型/检索版本、各指标分子分母、失败诊断和相对基线变化。

### V48/V49 真实回放与严格口径修正

2026-08-27 的 #43、#44、#45 曾显示替代推荐指标为 `8/8`，但该版本的断言允许目标轮候选集为空，只要没有与源轮重叠就会通过。因此该结论不成立，不能作为“换几家已修复”的证据。

V49 将评测口径改为：源轮和目标轮的推荐集合都必须非空，且两个集合完全不相交；同时把每轮实际推荐商户 ID 写入 `actual_recommendation_snapshots_json`，可在诊断接口直接复核。严格口径下的初次真实回放 #46/#47/#48 分别为主集 `0/4`、留出集 `0/3`、鲁棒集 `0/1`。根因是模型与用户输入使用“福州”，而商户表城市存储为“福州市”，检索层采用精确等值匹配后得到空候选。

修复在检索层为任意城市名同时查询带/不带“市”后缀的等价形式，不包含城市名单硬编码。随后 #49/#50/#51 的严格替代推荐指标为主集 `3/4`、留出集 `3/3`、鲁棒集 `1/1`。#49 的推荐快照进一步复现了连续换店缺口：`[73,61,66] -> [84,82,62] -> [73,66,78]`，第三轮复用了第一轮候选。

为此，Working Memory 新增同一检索域的已展示商户 ID 集合。每次推荐累积展示过的商户；用户表达搜索细化/换店时全部排除；城市、菜系、预算等检索域变化时与候选池一起清空，候选池本身仍仅保存最近一轮供相对条件推导。最终主集回归 #52：完成 `15/19`、错误率 `21.05%`、严格替代推荐 `4/4`。结合 #50/#51，当前三套数据集的替代推荐断言为 `8/8`，且已满足非空与无交集的严格定义。

本轮本地完整验证：`mvn -q test`，120 个测试通过、0 失败、0 错误、1 个既有跳过项。剩余评测失败仍集中在非餐饮闲聊路由、指代/工具覆盖、显式地点和设备位置仲裁，以及部分 Working Memory 期望；它们不应被归因为替代推荐缺陷，也不能据此宣称全套对话评测已通过。

## 2026-08-27：推荐数量请求解析与评测
...
## 2026-08-27：三层对话评测真实回放 #55/#56/#57

在已登录的本地运行环境，以真实 `deepseek-v4-flash`、MySQL、Redis、Milvus、持久化 Working Memory 和工具审计执行三层对话回放。评测不是 Mock：执行器为每条轨迹生成新的 `chatId`，逐轮调用与控制台相同的 `ChatOrchestrationService.chat()`，再从响应、工作记忆、商户表、工具审计表和决策指标表采集实际结果。当前启用数据集共 41 条轨迹：主集 19 条、留出集 16 条、鲁棒集 6 条。

### 判分口径

每条轨迹由数据库中的 `turns_json` 驱动，并只校验该用例声明过的断言：逐轮路由、上下文改写关键字段、工具名称/参数覆盖、最终状态、最终商户和所属城市、预期异常后的恢复路由、最小 Working Memory 字段，以及指定轮次推荐集合的“非空且无交集”。`completedCount` 的含义是“该轨迹所有已配置断言均通过”；任一断言失败即不计入完成。因此 `errorRate` 是轨迹断言失败率，不是 HTTP 错误率，也不是模型调用失败率。当前会话评测走同步入口，记录的是整条多轮轨迹耗时，尚未记录浏览器到首个 `text_delta` 的首字延迟（TTFT）。

| 运行 | 数据集 | 轨迹完全通过 | 路由 | 工具匹配/覆盖 | 地点/商户 | 最终状态 | 严格替代推荐 | 平均 / P95 轨迹耗时 | 模型调用 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| #55 | `conversation-v1` 主集 | 15/19 (78.9%) | 17/19 | 18/19，10/11 | 19/19，19/19 | 18/19 | 4/4 | 16542ms / 33885ms | 22/22 成功 |
| #56 | `conversation-holdout-v1` 留出集 | 12/16 (75.0%) | 15/16 | 15/16，5/6 | 16/16，16/16 | 13/16 | 3/3 | 15721ms / 26718ms | 18/18 成功 |
| #57 | `conversation-robustness-v1` 鲁棒集 | 1/6 (16.7%) | 5/6 | 6/6，1/1 | 6/6，6/6 | 6/6 | 1/1 | 18443ms / 24763ms | 9/9 成功 |

合计 28/41 条轨迹完全通过（68.3%），模型调用 49/49 成功；地点与最终商户校验为 41/41，严格替代推荐为 8/8。该合计不能表述为系统成功率：鲁棒集中的一条轨迹只要最终 Working Memory 与历史断言不一致，即使推荐、城市、状态和模型调用均正确，也会使整条轨迹不通过。

与相同数据集的上一轮基线 #52/#53/#54 相比：主集最终状态匹配率增加 5.26 个百分点、平均耗时减少 647ms；留出集各质量指标不变、平均耗时增加 952ms；鲁棒集工具匹配率增加 16.67 个百分点、平均耗时减少 5029ms。该比较只用于观察回归，不能据此宣称吞吐、并发或稳定性能力。

### 未通过项与加固方向

失败分类计数是断言维度计数，同一轨迹可能同时有多项失败，不能与失败轨迹数直接相加。

1. 主集 #55 有 4 条失败轨迹：
   - `NON_DINING_GENERAL_CHAT`：用户询问羽毛球馆，期望 `GENERAL_CHAT`，实际进入 `START_DECISION` 并进入地点澄清。应加固非餐饮意图的确定性拦截，避免本地生活泛查询误入餐饮决策链路。
   - `ANAPHORA_RESOLUTION_THIS_THAT`：路由和商户均正确，但期望评价证据工具 `search_shop_evidence`，实际调用 `get_shop_detail`。需要明确“评价/环境”事实查询的工具契约，并让工具选择与评测期望一致；不能仅因返回成功就视为通过。
   - `START_NEW_RECOM_CLEAR_INTENT`：期望福州川菜无候选后进入 `WAITING_RELAXATION`，实际因新增福建数据召回商户并 `COMPLETED`。这是数据扩充导致的期望过时，不应通过改代码强制返回无结果；应将用例改为接受 `COMPLETED|WAITING_RELAXATION`，或构造稳定的无候选条件。
   - `EXPLICIT_DESTINATION_OVERRIDES_DEVICE_LOCATION`：期望“命名地点解析 -> 确认事件”，实际为 `START_DECISION -> ERROR`，且出现 1 次未按预期恢复的执行异常。应复现并加固“重庆”等显式目的地的解析、确认事件和失败降级，确保不会误用设备位置或直接抛错。

2. 留出集 #56 有 4 条失败轨迹：
   - `ANAPHORA_CUISINE_ALIAS_REFERENCE`：同样出现“评价”被 `get_shop_detail` 执行而不是 `search_shop_evidence`，与主集是同一个工具契约缺口。
   - `NO_RESULT_RELAX_CUISINE_CATEGORY`：无 Taco 候选后，第二轮实际走 `EXPLAIN_SUSPENDED_DECISION`，未按用例预期发起放宽菜系的新推荐。需要明确用户表达“放宽/换一种”的触发语义，并补充对应变体回归。
   - `PAUSE_INTERRUPTED_BY_COFFEE`、`LOCATION_SWITCH_FUZHOU_TO_HANGZHOU`：路由和城市隔离正确，但最终进入 `WAITING_RELAXATION` 而非 `COMPLETED`。应分别补足精品咖啡、杭州东站附近的可检索数据，或把这类数据依赖用例的终态声明为 `COMPLETED|WAITING_RELAXATION`；不能把缺数据伪装为已完成推荐。

3. 鲁棒集 #57 有 5 条失败轨迹，均包含 Working Memory 断言不一致：
   - `ROBUST_CHAT_INTERRUPT_AND_RESUME` 的第三轮“继续，找安静一点的”实际被路由为 `BUSINESS_FOLLOW_UP`，而非新的 `START_DECISION`；需要提高“继续 + 新筛选条件”覆盖旧追问意图的优先级。
   - 其余四条以及上述轨迹的 Memory 失败，原断言要求最终阶段为 `WAITING_RELAXATION` 且候选池为空；福建数据扩充后实际均完成推荐并保留候选池。城市、商户、终态和异常恢复断言仍通过。应将鲁棒集的核心目标改为“旧条件/旧城市不泄漏、状态可恢复”，并为“有数据完成推荐”和“无数据等待放宽”分别构造稳定用例，而不是固定候选池为空。
   - `ROBUST_CITY_SWITCH_INVALIDATES_CANDIDATES` 已通过首末推荐集合无交集断言，说明本轮没有把杭州候选泄漏到福州；但其旧的空候选池 Memory 期望需要重基线。
   - 新增的 `ROBUST_REOPEN_CHAT_RESTORES_CONTEXT` 未出现在失败诊断：相同 `chatId` 重开后追问“刚才第一家评价如何”正确进入商户追问、执行 `search_shop_evidence`，并恢复地点、候选池和焦点商户。

后续优先级：先修复非餐饮误路由、显式目的地异常和“继续 + 新条件”路由；再统一评价类事实查询的工具契约；最后按新福建数据重建“稳定无候选”和“有候选完成”两类轨迹的期望。任何修复必须新增或更新对应评测轨迹，并再次记录运行 ID、分子分母、失败诊断和与基线的差异。

## 2026-08-27：Milvus 硬过滤白名单下推与真实回归 #58/#59/#60

此前虽然先在 MySQL 完成城市、区县、预算、菜系、距离、营业时间和已展示商户排除，但 Milvus 仍对全量向量索引执行 `Top 80` 检索，之后才在 Java 内按 MySQL 白名单取交集。这属于双路独立召回后融合，不是“硬过滤后再向量召回”；当白名单很小时，Top 80 中会包含大量无关城市或不合规商户的文档。

现在将 `hardMatched` 的商户 ID 集合构造成 Spring AI `FilterExpressionBuilder.in("shopId", ids)`，随 `SearchRequest` 下推至 Milvus。每条画像与评价文档本来就带有数值型 `shopId` metadata，因此无需改变数据模型；应用层原有的白名单与相似度阈值 `0.35` 校验继续保留，作为防御性边界。这样链路变为：MySQL 硬过滤 -> 将合规商户 ID 白名单下推至 Milvus -> 在白名单文档中按语义相似度取 Top K -> 每商户取最高分 -> 语义分按权重 `18` 融入确定性业务排序。距离、预算、营业时间等仍由 MySQL 作最终事实校验。

`MilvusSemanticShopRetrieverTest` 新增请求级断言，要求 `SearchRequest` 必须包含 `shopId` filter，避免后续改动悄然退化为全库向量检索；同时保留低分文档与白名单外高分文档均不参与加分的测试。`mvn -q -Dtest=MilvusSemanticShopRetrieverTest test` 和 `mvn -q -DskipTests compile` 通过。为验证运行数据，重启本地服务并重建 Milvus 索引，结果为 980 家商户、3180 条文档。真实日志显示当硬过滤候选为 3 家时，Milvus 只返回 21 条文档；候选为 12 家时返回 63 条，证明白名单已在查询端生效，而不是仅在 Java 端丢弃结果。

随后以相同模型、数据集和真实依赖回放 #58/#59/#60，并与 #55/#56/#57 对比：

| 数据集 | 运行对比 | 完全通过 | 路由 | 工具匹配/覆盖 | 地点/商户 | 最终状态 | 替代推荐 | 平均轨迹耗时变化 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 主集 | #58 vs #55 | 15/19 -> 15/19 | 17/19 -> 17/19 | 18/19、10/11 -> 不变 | 19/19、19/19 -> 不变 | 18/19 -> 不变 | 4/4 -> 4/4 | +409ms（16951ms） |
| 留出集 | #59 vs #56 | 12/16 -> 12/16 | 15/16 -> 15/16 | 15/16、5/6 -> 不变 | 16/16、16/16 -> 不变 | 13/16 -> 不变 | 3/3 -> 3/3 | -646ms（15075ms） |
...
2. **Snapshot 使用生产等价 pre-filter**：原始实现使用全库搜索 + Python 过滤，但这两者不等价。修正后使用 Milvus REST API v2 的 `filter` 参数（JSON 路径语法 `metadata["shopId"] in [...]`），该参数是真正的 pre-filter。
...
Milvus REST API v2 的 `filter` 参数是 pre-filter（非 post-filter）。验证方法：同一查询（"人均150的日料"）对比 pre-filter 与全库搜索 + Python 过滤：
- Pre-filter: 80 条结果，来自 24 家 shop
- 全库搜索 + Python 过滤: 1 条结果，来自 1 家 shop
- 结论：全库搜索 + Python 过滤不等价于 pre-filter
...
|--------|:------:|:-----:|:---------:|
| Recall@3 | 0.7291 | 0.7614 | 0.7722 |
| NDCG@3 | 0.9355 | 0.9558 | 0.9704 |
...
1. 语义分数减少 8% 的 ranking 错误（39→36），但 Top1 Accuracy 不受 weight=18 影响
2. 仅 6% 的 GT shop 没有语义信号（从 v2 的 69% 大幅下降）
...
| 变化 | v2（全库搜索 + Python 过滤） | v3（pre-filter） |
|------|:---:|:---:|
...
v3 的 pre-filter 方案与生产等价。语义信号更有用因为 ANN 搜索被限制在 allowed shop 集合内。
...
| `retrieval_ablation_milvus_snapshot.py` | Milvus 语义分数快照（document-level 原始结果，pre-filter） |
...
### 21. 词表冻结声明 + 统计层一致性根治（第 6 处双真相根治，2026-09-03）

**战略判断（词表冻结）**：step2.5 后规则层到达效率前沿——功能 case MODEL 兜底 9/46 → 2/46（4.3%），剩余 2 个 turn 是"你是谁/写论文"真 LLM 领地（规则永远不该碰）。**下一个词的边际收益趋近于零（没有"该接没接"存量），边际成本在上升（每词都是新碰撞面）**。声明：**词表进入冻结状态，只接受 bug 驱动的新增，不做主动扩展**。收束句："我把 LLM 兜底占比从 20.5% 压到 4.3% 后停止词表扩展——不是因为没词可加了，是因为剩余样本证明下一个词的期望收益为负。"

**统计层双真相根治（GLM 完整性检查）**：case3 的误标修复只修了 isNonDiningDomain 一处，但 route() 其他确定性分支同样可能 source 误标。审计后补齐：
1. **L184 LOCATION_RESOLUTION / L194 EXPLAIN_SUSPENDED / L202 位置恢复 / L214 replacesPausedDecision** 拦截时均显式 `assessment.setSource("RULE")`；
2. **resolveContextualFollowUpRoute 返回确定性 BFU 时**，若 source 仍为 MODEL 则校正为 RULE（该函数是 ShopInquiry/候选引用的确定性路由，非 LLM 兜底）；
3. **一致性校验**：聚合脚本输出全部 MODEL turn 清单人工核对（功能 case 应只剩真 LLM 领地）。根治后 run80 校验通过：功能 MODEL 仅 case8（开放问答），探针/对抗的 MODEL 均为刻意 miss 的改述与真歧义，无"规则该接没接却落 MODEL"漏网。

**碰撞测试（GLM 推断的具体路径）**："有什么"进入 isNewRecommendationIntent 后，"这家店有什么优惠"含 ShopInquiry 信号（优惠）但判定晚于正向规则——验证 isFocusedShopQuestion（"这家"）在 isNewRecommendationIntent 入口 return false 兜底，补测试 `shopInquiryWithYouShenMeNotSwallowedByStartDecision` 钉住（"这家店有什么优惠/评价"→ false、"有什么优惠"→ false）。

**对抗/歧义探针（run80，40/40 全绿）**：补 6 条两意图模糊表述（"火锅怎么样""有没有近一点的""还有吗"为词表 miss 真歧义 → 落 LLM 且路由正确，验证"LLM 提议+状态机裁决"真实行为；"那家评分多少""换一家贵的""这家怎么样"被规则接住验证边界）。**探针集改为有机生长机制**：未来每个路由 bug 的表述变体进探针集（bug→回归模式），不做一次性批量扩（待修 #45 一致性断言自动 fail-fast）。

**叙事修正（重要）**：面试主线措辞从"GLM 说'这类 turn 都是 LLM 领地——验证过吗'"改为**"评审方只要求我逐条验证，我验证后发现推翻的是我自己的先验（上轮我自己判断'存量 9 个 turn 都是规则不该覆盖的 LLM 领地'，逐条审计后 78% 是规则该接没接）"**——挑战→自查→推翻自己，比"AI 说错了我纠正 AI"更能证明判断力归属在自己，且事实口径经得起对质（AI 当时原话是"这 9 个我看不到具体内容，不下结论，但你该逐条问同一个问题"）。
...
### Structured Understanding Shadow 失败归因（2026-09-10）

基于已归档 Run153/154 做第二阶段离线分析，没有重跑评测、调用 LLM 或修改业务 Validator。177 个 Turn 中 Structured invoked 143 个；真实 `modelCalls[].purpose` 的精确 Legacy shape 为 `EXTRACTION` 120、`ROUTING` 8、`ROUTING+EXTRACTION` 15、无语义调用 34。15 个 multi-call Turn 全部是 `ROUTING+EXTRACTION`，没有观测到 `REWRITE+EXTRACTION` 或 `REWRITE+ROUTING`，说明当前需要优先理解的是 Routing escalation 后仍需 Extraction 的组合，而不是把所有 Semantic Authority 无差别合并。

Evidence 离线分类（Run153/154 合并）为：exact span 312、text 唯一但 offset 错 83、多次出现 0、text 不存在 0、evidence 缺失 6；另有 23 个 Turn 属于非 evidence 的 Schema/enum/reference 或无法归入 offset 的错误。当前 Valid Rate 为 52.45%（75/143）。采用 `OFFLINE_COUNTERFACTUAL` 协议——LLM 只提供 evidence.text，Java 对原文唯一匹配并生成 authoritative span——可恢复 39 个仅 offset 错误 Turn，反事实 Valid Rate 为 79.72%（114/143），仍有 29 个 Turn invalid。该结果是离线诊断，不是线上指标；Python Unicode 索引不能成为生产事实源。

Trigger 成本进一步拆分：`ROUTING_ESCALATION` 23 Turn，Legacy 平均 latency 5.96s、Structured 8.85s；其中仅 Routing 的 8 Turn 为 0.44s 对 4.42s，Routing 后继续 Extraction 的 15 Turn 为 8.90s 对 11.21s。`CONSTRAINT_EXTRACTION` 120 Turn，Legacy 平均 8.19s、Structured 12.12s。Structured Full IR 在 Extraction 已确定 action 的场景仍要求 acts、references、query、shopFact 和完整 evidence；IR 平均约 1.46 acts、0.10 references、0.98 delta、3.31 evidence、417 serialized chars，completion token 与 evidence 数/IR 字符的相关系数仅作诊断观察（0.5535/0.5772），不作严格因果结论。

基于 multi-call composition、evidence 反事实和 trigger 成本，下一版推荐方案 B：capability-scoped Structured Understanding。Routing escalation 只请求当前未知的 routing semantic；Constraint Extraction 只请求 criteria semantic；若 Routing 后还需要 Extraction，应复用同一结果而不新增第二次调用。Evidence Protocol V2 建议保留为下一轮设计：模型只输出 `evidence.text`，Java 唯一匹配时生成 `ResolvedEvidence`，多匹配标记 ambiguous、不猜，text 不存在仍 fail-closed。方案 A 的 Full IR 无差别统一缺少必要性和成本证据，方案 C 不能覆盖 Routing+Extraction 组合，方案 D 则放弃已获得的结构化观测价值。本轮只完成归因，未实施方案 B 或 Evidence Protocol V2。

### Structured Understanding V2：Routing Fusion 收缩实验（2026-09-10）

基于实验分支 `feat/structured-understanding-experiment` 的 `73903998fd835e4911354749e94e5531aec3479d`，按离线归因结果将 Structured Understanding 收缩为 capability-scoped V2：只有既有 Java 路由和上下文快速路径都无法决定、即将进入 `ROUTING_ESCALATION` 时才按 request-scoped lazy-once 调用；确定性的 `START_DECISION` 与 Criteria Extraction 继续由 legacy `ConstraintExtractor` 负责，不再为 extraction-only Turn 调用 V2。V2 调用复用现有轻量 Routing provider 配置（provider/baseUrl/model/key/timeout），没有新增第二套模型配置。

V2 的单次 tool schema 只允许 `REQUEST_RECOMMENDATION`、`MUTATE_CRITERIA`、`EXPLORE_ALTERNATIVE`、`CHITCHAT_OR_UNKNOWN` linguistic acts、raw `criteriaDelta`、raw `locationExpression` 和 `ambiguities`。不包含 shop/session identity、reference、shop fact/query、最终 action/state、policy、tool 或 start/end offset；Java Adapter 只在无地点 authority、无 ambiguity、act/delta 可安全映射时以 `START_DECISION` 接管，否则 fail-closed 回到 legacy routing/extraction。Routing 与 criteria delta 在同一个 V2 结果中复用，不再为同一 Turn 发起第二个 Structured 调用。
