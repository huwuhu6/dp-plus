...
2026-08-27，在已登录用户 `1027`、模型 `deepseek-v4-flash` 的本地真实依赖环境中提交新运行：主集 #43、留出集 #44、鲁棒集 #45。替代推荐不重复指标全部通过：主集 `4/4`、留出集 `3/3`、鲁棒集 `1/1`，合计 `8/8`。这证明本轮新增的“换几家/再推荐不同的/还有别的餐厅/换一批”、连续换店、商户追问后换店及跨城候选隔离断言在真实运行中均未复用已展示商户。

其他指标必须按数据集分别阅读：#43 为 19 条轨迹，完成 `14/19`，路由 `15/19`，上下文改写 `6/7`，工具覆盖 `7/11`，最终状态 `18/19`，平均/P95 耗时 `14838/23089ms`；#44 为 15 条，完成 `10/15`，路由 `13/15`，上下文改写 `3/3`，工具覆盖 `4/6`，最终状态 `10/15`，平均/P95 `14604/26698ms`；#45 为 5 条，完成 `4/5`，路由 `5/5`，最终状态 `4/5`，平均/P95 `17591/20780ms`。三次运行的模型调用失败数均为 0。

未通过项不能归因于本轮替代推荐修复：主集仍有非餐饮闲聊路由、指代解析工具覆盖、显式目的地覆盖设备定位、组合优惠/证据问答和“商户追问后换店”路由/工具期望差异；留出集仍有菜系别名指代、优惠后新推荐、零结果菜系放宽、暂停后咖啡新需求和福州到杭州位置切换的最终状态或路由差异；鲁棒集仅 `ROBUST_INVALID_ACTION_THEN_RECOVER` 的最终状态与 Working Memory 断言失败。下一轮应按这些失败用例逐项定位和修复；不得因为不重复指标 `8/8` 而宣称整套对话评测已通过。
